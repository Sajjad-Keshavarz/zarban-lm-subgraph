...
export function handleReserveInterestRateStrategyChanged(
  ...
  if (reserve.zToken == zeroAddress().toHexString()) {
    return;
  }
...
export function handleZTokenUpgraded(event: ZTokenUpgraded): void {
  let zToken = getOrInitZToken(event.params.proxy);
  zToken.tokenContractImpl = event.params.implementation;
  zToken.save();
}

export function handleStableDebtTokenUpgraded(event: StableDebtTokenUpgraded): void {
  let sToken = getOrInitZToken(event.params.proxy);
...
export function handleVariableDebtTokenUpgraded(event: VariableDebtTokenUpgraded): void {
  let vToken = getOrInitZToken(event.params.proxy);
