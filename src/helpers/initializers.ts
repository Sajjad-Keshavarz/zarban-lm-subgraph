...
function initUserReserve(
  ...
  if (userReserve === null) {
    ...
    userReserve.scaledZTokenBalance = zeroBI();
    userReserve.scaledVariableDebt = zeroBI();
    userReserve.principalStableDebt = zeroBI();
    userReserve.currentZTokenBalance = zeroBI();
    ...
    // incentives
    userReserve.zTokenincentivesUserIndex = zeroBI();
...
export function getOrInitReserve(underlyingAsset: Bytes, event: ethereum.Event): Reserve {
  ...
  if (reserve === null) {
    ...
    reserve.totalZTokenSupply = zeroBI();
    ...
    reserve.zToken = zeroAddress().toHexString();
    ...
    reserve.zTokenIncentivesIndex = zeroBI();
...
export function getOrInitZToken(zTokenAddress: Bytes): ZToken {
  let zTokenId = getAtokenId(zTokenAddress);
  let zToken = ZToken.load(zTokenId);
  if (!zToken) {
    zToken = new ZToken(zTokenId);
    zToken.underlyingAssetAddress = new Bytes(1);
    zToken.tokenContractImpl = zeroAddress();
    zToken.pool = '';
    zToken.underlyingAssetDecimals = 18;
  }
  return zToken as ZToken;
}
...
export function getOrInitReserveParamsHistoryItem(
  ...
  if (!reserveParamsHistoryItem) {
    ...
    reserveParamsHistoryItem.totalZTokenSupply = zeroBI();
