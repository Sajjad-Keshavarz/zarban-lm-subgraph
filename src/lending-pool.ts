...
export function handleFlashLoan(event: FlashLoan): void {
  ...
  poolReserve.totalZTokenSupply = poolReserve.totalZTokenSupply.plus(premium);
...
export function handleReserveDataUpdated(event: ReserveDataUpdated): void {
  ...
  if (timestamp.gt(prevTimestamp)) {
    let growth = calculateGrowth(
      reserve.totalZTokenSupply,
      reserve.liquidityRate,
      prevTimestamp,
      timestamp
    );
    reserve.totalZTokenSupply = reserve.totalZTokenSupply.plus(growth);
